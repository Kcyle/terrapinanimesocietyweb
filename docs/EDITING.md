...
This guide covers every content file on the site and explains what each field does. The main README covers the basics of making a change, finding text, and publishing, so I would read that first if you have not.

Every file described here is in `src/data/` and is plain text. You do not need to write any code to change what the website says.
...
If a build fails after you edit, check the file you just changed. It is most likely missing comma or quotation mark.

## The screening schedule

This is in `src/data/screenings.json`, and it is the single file that controls the anime lineup on both the home page and the meetings page, along with the meeting details.
...
This is in `src/data/about.json` and fills the page at `/about`. It holds the heading, the opening description, the paragraphs of body text, the button label, and the two sets of photographs. The paragraphs are a plain list, so you can add one, remove one, or reorder them.
...
These are in `src/data/activities.json`, which holds the Activities heading, the paragraph underneath it, and every activity card.
...
These are in `src/data/partners.json`, which fills the Partners section on the home page.
...
These are in `src/data/kamecon.json` and `src/data/terpcon.json`. The event details sit at the top of each file, so the date, time, location, description, and links are all in one place. Below them is the vendor list, where each vendor is one block:
...
This are in `src/data/home.json`, which contains the large heading, the club description below it, the buttons, the whole About section, the photo gallery, and the list of images in the rotating card carousel.
...
These are in `src/data/site.json`, which holds the things that appear on more than one page.
...
Images are in `public/images/`, sorted into folders by purpose: Backgrounds, Photos, Cards, Characters, Vendors, Artwork, and Icons.
...
These are in `src/styles/global.css`. The colors, fonts, and spacing used across the site are defined once as variables at the top of the file, on the lines beginning with `--`. Changing a value there updates it everywhere, so you never need to find and replace the same color in several files.

## Finding things quickly

Rather than guessing which file holds a piece of text, search for the text itself. Press `/` on GitHub, or Ctrl and Shift and F in VS Code, then paste in a sentence exactly as it appears on the are site. The results take you straight to the file and line that produces it.
