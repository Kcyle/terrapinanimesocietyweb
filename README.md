...
To add it, create a free account and a new project at supabase.com, open the SQL Editor in your project, and create your tables. Turn on Row Level Security on every table, which is covered below and is not optional. Go to Settings, then API, and copy the Project URL and the anon public key. Install the client library with `npm install @supabase/supabase-js`. Store the two values as `PUBLIC_SUPABASE_URL` and `PUBLIC_SUPABASE_ANON_KEY`, since Astro only exposes a variable to the browser when its name begins with `PUBLIC_`. Then add the same two values as repository secrets under Settings, then Secrets and variables, then Actions, and reference them in the build step of `.github/workflows/deploy.yml`. Without that last step the deployed site will not have them even though it works on your own computer. Here's a quick video on getting started: https://www.youtube.com/watch?v=i_bPeTZVlg0
...
The simplest thing to do is to transfer the repository to a GitHub organization owned by the club, as described above. If you do that, any eboard member can just login to the clubs github account and add their own github accounts as a collaborator. **You should never transfer ownership of this repository to an individual unless absolutely necessary.** Doing so breaks the custom domain until UMD updates the DNS record, and it leaves the club dependent on one person's personal account.
